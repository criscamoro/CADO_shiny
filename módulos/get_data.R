...
data_process <- function(x) {
  datos_amb <- as_tibble(read.xlsx(x[1], sheet = x[2], detectDates = T)) %>%
    ...
    mutate(idParametro = as.character(mgsub(as_tibble(read.xlsx(x[1], sheet = 'Parametros'))$idParametros, 
                                            as_tibble(read.xlsx(x[1], sheet = 'Parametros'))$param, idParametro))) %>%
    mutate(idPuntoMuestreo = as.character(mgsub((as_tibble(read.xlsx(x[1], sheet = x[3], fillMergedCells = T)) %>% 
