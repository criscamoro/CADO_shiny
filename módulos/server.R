...
server <- function(input, output) {
  ...
  output$a <- renderUI(paste(first(first(str_split(input$tabs, '_'))), '.amb.tidy', sep = ''))
  
  caji.amb.tidy.cre <- reactive({
    data <- caji.amb.tidy
    if (input$año != 'Todos') {
  ...
  output$dl.cre <- downloadHandler(
    ...
    content = function(file){
      write.csv(caji.amb.tidy.cre(), file, row.names = F)
    }
  )
